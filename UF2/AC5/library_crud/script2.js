...
async function deleteBook(event) {
    ...
    // Hacemos la petición de DELETE a la API pasando un FormData en el cuerpo del mensaje
    ...
    newBook.append("id",Number(id));

    let response = await fetch(apiUrl, {
        method: "DELETE",
        body: new URLSearchParams(newBook)
        // body: newBook
    });
...
async function editBook(event) {
    ...
    let newBook = new FormData();
    newBook.append("id",Number(id));
    newBook.append("title",titulo);
    newBook.append("author",autor);
    newBook.append("year",ano);
